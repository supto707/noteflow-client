...
const features = [
  ...
  "Unlimited databases",
  "All views — table, board, calendar, gallery",
  "Web publishing",
  "Nested pages (infinite depth)",
  "Real-time collaboration (up to 10 guests)",
  "Templates library",
  "File uploads (up to 5 MB each)",
        ...
        <motion.div style={{ opacity }} className="max-w-7xl mx-auto px-6 lg:px-12 text-center">
          <FadeUp delay={0.05} className="flex justify-center mb-10">
            <div className="inline-flex items-center gap-2 rounded-full border" style={{ padding: "7px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 500, color: "#6357E8", background: "rgba(99,87,232,0.08)", borderColor: "rgba(99,87,232,0.22)" }}>
              <div className="w-1.5 h-1.5 rounded-full bg-[#6357E8]" /> Free forever — no trials, no paywalls <ChevronRight size={13} />
            </div>
          </FadeUp>

          <div className="mb-8" style={{ lineHeight: 0.92 }}>
            <RevealLine delay={0.1}>
              <h1 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: "clamp(4rem, 12vw, 11rem)", fontWeight: 800, letterSpacing: "-0.04em", color: fg, lineHeight: 0.95 }}>Free.</h1>
            </RevealLine>
            <RevealLine delay={0.18}>
              <h1 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: "clamp(4rem, 12vw, 11rem)", fontWeight: 800, letterSpacing: "-0.04em", color: "#6357E8", lineHeight: 0.95, marginTop: "0.06em" }}>Forever.</h1>
            </RevealLine>
          </div>
          ...
          <FadeUp delay={0.42}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "clamp(1rem, 2vw, 1.25rem)", color: sub, maxWidth: 500, margin: "0 auto", lineHeight: 1.7 }}>
              No trials. No paywalls. No credit card. NoteFlow is completely free — now and always.
            </p>
          </FadeUp>
        </motion.div>
      </section>
      ...
      <section style={{ padding: "100px 0", background: bg }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <FadeUp className="flex items-center justify-center gap-3 mb-12">
            ...
            <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: 11, color: "#6357E8", letterSpacing: "0.1em", textTransform: "uppercase" }}>What you get</span>
            <div className="h-px bg-[#6357E8]" style={{ width: 28 }} />
          </FadeUp>
          <RevealLine>
            <h2 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: "clamp(3rem, 7vw, 6.5rem)", fontWeight: 800, letterSpacing: "-0.04em", color: fg, lineHeight: 0.95, marginBottom: "0.12em", textAlign: "center" }}>Everything included. No hidden upgrades.</h2>
          </RevealLine>

          <FadeUp delay={0.2}>
            <div style={{ padding: 48, background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 24, maxWidth: 900, margin: "32px auto 0" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 32 }}>
                <div style={{ width: 56, height: 56, borderRadius: 14, background: "rgba(99,87,232,0.14)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Sparkles size={24} style={{ color: "#6357E8" }} />
                </div>
                <div>
                  <div style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 28, fontWeight: 800, color: fg }}>Free Plan</div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 16, color: sub, marginTop: 4 }}>$0 / month — forever</div>
                </div>
              </div>
                ...
                {features.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div style={{
                      ...
                      background: "rgba(99,87,232,0.14)", flexShrink: 0,
                      display: "flex", alignItems: "center", justifyContent: "center"
                    }}>
                      <Check size={12} style={{ color: "#6357E8" }} />
                    </div>
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: sub }}>{item}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginTop: 40 }}>
                <Link to="/signup" className="inline-flex items-center gap-2 rounded-full group transition-all duration-300"
                  style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 16, fontWeight: 600, padding: "16px 40px", background: "#6357E8", color: "white", textDecoration: "none" }}>
